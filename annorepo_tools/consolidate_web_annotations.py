    ...
    for k, v in entity_index.items():
        if "relation" in v:
            if isinstance(v["relation"], list):
                for i, rel in enumerate(v["relation"]):
                    if "ref" in rel:
                        ...
                        if ref_key in entity_index:
                            label = label_of(entity_index[ref_key])
                            entity_index[k]["relation"][i]["label"] = label
                        else:
                ...
                if ref_key in entity_index:
                    label = label_of(entity_index[ref_key])
                    entity_index[k]["relation"]["label"] = label
                else:
                    print(f"WARNING: {k.replace('/', '.xml#')}: no entity found for ref=\"{original_ref}\"",
                          file=sys.stderr)
...
def label_of(entity_index: dict[str, dict[str, Any]]) -> str:
    if "label" in entity_index:
        label = entity_index["label"]
        if isinstance(label, str):
            return label
        elif isinstance(label, dict):
            return list(label.values())[0]
        # ic(label)
        return ""
    elif "displayLabel" in entity_index:
        return entity_index["displayLabel"]
    else:
        # ic(entity_index)
        return ""
...
def main():
    parser = argparse.ArgumentParser(
        description="Consolidate multiple webannotations, as produced by e.g. stam fromxml, by merging secondary ones into the primary one. This means that what were two annotations in the input, becomes one annotation with multiple targets in the output. This is intended for situations where the annotation body is identical. The script uses standard input and standard output (JSONL). Note that only annotations with identifiers can be merged (with their counterparts carrying a certain ID suffix)! All web annotations passed must describe the same resource")
    parser.add_argument("--new-type", action="store", type=str,
                        help="The type of the web annotation body when a secondary annotation was found and merged into a primary one",
                        default="NormalText")
    parser.add_argument("--original-type", action="store", type=str,
                        help="The type of the web annotation body when no secondary annotation was found",
                        default="OriginalText")
    parser.add_argument("--id-suffix", action="store", type=str,
                        help="The ID suffix secondary annotations carry, when compared to the primary ID",
                        default=".normal")
    parser.add_argument("--apparatus-dir", action="store", type=str,
                        help="Directory containing apparatus JSON files. These will be embedded in the web annotations whenever there is an occurrence of `tei:ref`. This is only a TEMPORARY measure for backward compatibility, it results in invalid/unformalised linked data!")
    parser.add_argument("--body-id-prefix", action="store", type=str,
                        help="Generate body IDs when absent, using the following prefix followed by a sequence number")
    parser.add_argument("--no-pass", action="store_true",
                        help="Ignore all annotations without IDs, do not pass them through")
    ...
    for id, webannotation in webannotations.items():
        if not id.endswith(args.id_suffix):  # not a secondary suffix
            if id.endswith((".translation-source", ".translation-target", "-translated", "-translationsource")) or (
                    'body' in webannotation and 'https://w3id.org/stam/extensions/stam-translate/Translation' in
                    webannotation['body']):
    ...
    print(f"Consolidated {merged} (out of {potential}) annotations, passed {len(pass_annotations)}, skipped {skipped}",
          file=sys.stderr)
